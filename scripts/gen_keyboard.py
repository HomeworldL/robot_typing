...
def add_collision_elements(root, name, coords, final=False):
    ...
    for key, (x, y, z, width, height) in coords.items():
        ...
        site = ET.Element(
            "site",
            name=f"sensor_site_{key}",
            type="box",  # 定义站点形状为立方体
            pos=f"{x:.4f} {y:.4f} {z - 0.001:.4f}",  # Z轴提升0.001m避免接触面重叠
            size=f"{height:.4f} {width:.4f} 0.004",  # MuJoCo站点尺寸为半长宽高
            rgba="0.3 0.3 0.8 0.5",  # 半透明蓝色可视化
        )
        ...
        #     "force",
        #     name=f"force_{key}",
        #     site=f"sensor_site_{key}",
        #     cutoff="1000",
        #     noise="0.01"  # 添加1cm位置噪声
        # )
...
def save_xml(root, output_path):
    ...
    # 解析并美化
    parsed = minidom.parseString(xml_str)
    pretty_str = parsed.toprettyxml(indent="  ", encoding="utf-8").decode("utf-8")
    
    # 正则替换多余空行：将连续两个换行符替换为一个
